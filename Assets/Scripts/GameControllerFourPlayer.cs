...
public class GameControllerFourPlayer : MonoBehaviour {
    ...
    private Vector3 player3Start;
    private Vector3 player4Start;
    ...
    void Start () {
        ...
        player2Wins = 0;
        timeDuration = 1.5f;
        player1Start = new Vector3(5.0f, .01f, -5.0f);
        player2Start = new Vector3(-5.0f, .01f, -5.0f);
        player3Start = new Vector3(5.0f, .01f, 5.0f);
        player4Start = new Vector3(-5.0f, .01f, 5.0f);
    ...
    IEnumerator LoadMatch()
    {
        ...
        else
        {
            ...
            Player3.transform.position = player3Start;
            Player4.transform.position = player4Start;
            canMove = false;
	...
	void Update () {
        ...
        else
        {
            ...
            else if (Player3.transform.position.y <= -7 && !matchOver)
            {
                matchOver = true;
                player2Wins++;
                prevWinner = 2;
                GameObject death = Instantiate(BloodSplatter, Player3.transform.position, Quaternion.identity);
                currentRound++;
                StartCoroutine(LoadMatch());
            }
            else if (Player4.transform.position.y <= -7 && !matchOver)
            {
                matchOver = true;
                player1Wins++;
                prevWinner = 1;
                GameObject death = Instantiate(BloodSplatter, Player4.transform.position, Quaternion.identity);
                currentRound++;
                StartCoroutine(LoadMatch());
            }
        }
